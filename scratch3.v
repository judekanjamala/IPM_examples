...
Inductive SFFF :=
    ...
    | F2
    | F3
    | BT. (* The invalid element "bottom" *)

Canonical Structure SFFFRAC := leibnizO SFFF.


Global Instance SFFFRAop : Op SFFF :=
  λ x y, match x, y with
           | F2, F2 => F2
           (* | F1, F1 => F1 *)
           | _, _ => BT end.

Global Instance SFFFRAValid : Valid SFFF :=
  λ x, match x with
       | ST | F1 | F2 | F3 => True
       | _ => False end.

Global Instance SFFFRACore : PCore SFFF :=
  λ _, None.

(* Produce cameras from the RA *)
Definition SFFFRA_mixin : RAMixin SFFF.
Proof.
    split; try apply _; try done.
    (* The operation is associative. *)
    - unfold op, SFRAop. intros [] [] []; reflexivity.
...
Canonical Structure SFFFRA := discreteR SFFF SFFFRA_mixin.

Global Instance SFFFRA_cmra_discrete : CmraDiscrete SFFFRA.
Proof. apply discrete_cmra_discrete. Qed.

Global Instance SFFFRA_ST_exclusive : Exclusive ST.
Proof. intros x. done. Qed.

Global Instance SFFFRA_F1_exclusive : Exclusive F1.
Proof. intros x. done. Qed.

(* Global Instance SFFFRA_F2_exclusive : Exclusive F2.
Proof. intros x. done. Qed. *)

(* States the duplicability of F2.  *)
Lemma SFFFRA_FF: F2 = F2 ⋅ F2.
Proof.
  done.
Qed.

Lemma SFFFRA_update_1 : (ST : SFFF) ~~> F1.
Proof.
  apply cmra_update_exclusive.
  done.
  (* *** Manual proof *** *)
...
Lemma SFFFRA_update_2 : (F1 : SFFF) ~~> F2.
Proof.
  apply cmra_update_exclusive.
  done.
  (* *** Manual proof *** *)
...
Lemma SFFFRA_update_4 : (F2 : SFFF) ~~> F1.
Proof.
  (* apply cmra_update_exclusive. *)
  (* done. *)
  (* *** Manual proof *** *)
...
Class SFFFG Σ := SFFF_G :> inG Σ (SFFFRA).
...
Lemma own_SFFFRA_FF (γ: gname): own γ F2 ⊣⊢ own γ F2 ∗ own γ F2.
Proof.
  apply (own_op _ F2 F2).
  (* Alternative proof using lemmas from above *)
  (* rewrite -own_op. *)
  (* rewrite -SFRA_FF. *)
  (* reflexivity. *)
...
(* This lemma states the incompatibility of ST and F1 *)
Lemma own_SFFF_false_1 (γ: gname): own γ ST ∗ own γ F1 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
(* This lemma states the incompatibility of ST and F2 *)
Lemma own_SFFF_false_2 (γ: gname): own γ ST ∗ own γ F2 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
(* This lemma states the incompatibility of ST and F3 *)
Lemma own_SFFF_false_3 (γ: gname): own γ ST ∗ own γ F3 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
(* This lemma states the incompatibility of F1 and F2 *)
Lemma own_SFFF_false_4 (γ: gname): own γ F1 ∗ own γ F2 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
(* This lemma states the incompatibility of F1 and F3 *)
Lemma own_SF_false_5 (γ: gname): own γ F1 ∗ own γ F3 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
(* This lemma states the incompatibility of F2 and F3 *)
Lemma own_SF_false_6 (γ: gname): own γ F2 ∗ own γ F3 ⊢ False.
Proof.
  rewrite -own_op.
  rewrite own_valid.
  iIntros (Contra).
...
Definition incr_inv (ℓ : loc) (γ1 γ2 γ3 : gname) (n : Z) : iProp :=
    ℓ ↦ #n ∗ own γ1 S  ∗ own γ2 ST 
  ∨ ℓ ↦ #(n+1) ∗ own γ1 F ∗ own γ2 F1 ∗ own γ3 (1/3)%Qp
  ∨ ℓ ↦ #(n+2) ∗ own γ1 F ∗ own γ2 F2 ∗ own γ3 (2/3)%Qp
  ∨ ℓ ↦ #(n+3) ∗ own γ1 F ∗ own γ3 1%Qp.
...
Lemma div3_1_2_1  : ((1 / 3 + 2 / 3 = 1)%Qp)%I.
Proof.
    Admitted.

(* Proving a spec for the threads *)
Lemma incr_spec (ℓ : loc) (n : Z) (γ1 γ2 γ3 : gname):
  {{{ inv N (incr_inv ℓ γ1 γ2 γ3 n) ∗ own γ3 (1 / 3)%Qp }}}
    incr ℓ
  {{{ v, RET #v; own γ1 F }}}.
Proof.
  ...
  iApply ((wp_wand _ _ _ (fun v => (⌜v = #n⌝ ∨ 
                                    ⌜v = #(n+1)⌝ ∗ own γ1 F ∨ 
                                    ⌜v = #(n+2)⌝ ∗ own γ1 F ∗ own γ2 F2) ∗ 
                                        own γ3 (1 / 3)%Qp)%I with "[Own1/3]")).
  { (* The first triple about the result of !ℓ *)
    ...
    - (* Case 1: l ↦ #n ∗ own γ1 S ∗  own γ2 ST *)
      ...
      iMod ("Iclose" with "[Hl Own]") as "_".
      { iLeft. iFrame. }
      iFrame. iLeft. done.
    - (* Case 2: l ↦ #(n + 1) ∗ own γ1 F ∗ own γ2 F1 ∗ own γ3 (1 / 3)%Qp *)
      (* This is analogous to the first case. *)
      iDestruct "I" as "(Hl & OwnF & OwnF1 & Own1/3_I)".
      ...
      wp_load.
      iMod ("Iclose" with "[Hl OwnF_1 OwnF1 Own1/3_I]") as "_".
      { iRight. iLeft. iFrame. }
      iFrame. iRight. iLeft. iFrame. done.
      - (* Case 3: l ↦ #(n + 2) ∗ own γ1 F ∗ own γ2 F2 ∗ own γ2 (2 / 3)%Qp *)
      (* This is analogous to the first case. *)
      iDestruct "I" as "(Hl & OwnF & OwnF2 & Own2/3_I)".
      (* Similar to previous case, but there are two Fs to duplicate. *)
      iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
      iDestruct (own_SFFFRA_FF with "OwnF2") as "[OwnF2_1 OwnF2_2]".
      wp_load.
      iMod ("Iclose" with "[Hl OwnF_1 OwnF2_1 Own2/3_I]") as "_".
      { iNext. unfold incr_inv. iRight. iRight. iLeft. iFrame. } iModIntro.
      iFrame. iRight. iFrame. iRight. iFrame.  done.
    - (* Case 4: l ↦ #(n + 3) ∗ own γ1 F ∗ ∗ own γ2 F3 ∗ own γ2 1%Qp
         This state is impossible, as it means we have already incremented twice, represented by
         the ownership of 1. In this thread we have not written to l yet (represented by
         still owning 1/3, which we would lose when closing the invariant in a new state ),
         l could thus have at most been updated twice (by the other 2 threads). We get the contradiction
  ...
  { (* The second triple about storing the incremented v *)
    ...
    iDestruct "Hv" as "[-> | [[-> OwnF] | (-> & OwnF & OwnF2)]]";
    wp_binop;
    ...
    - (* Case Storing (n + 1), invariant was in state S ST *)
      ...
      iDestruct "Own" as "[OwnS OwnST]".
      iMod (own_update _ _ F with "OwnS") as "OwnF"; first (apply SFRA_update).
      iMod (own_update _ _ F1 with "OwnST") as "OwnF1"; first (apply SFFFRA_update_1).
      (* We then duplicate F, because we need to pass one F to the conclusion too. *)
      (* iMod (own_alloc S) as (γ2) "OwnS"; first done. *)
      iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
      iMod ("Iclose" with "[Hl OwnF_1 OwnF1 Own1/3]") as "_".
      
      { iRight. iLeft. iFrame. }
      iApply ("Post" with "OwnF_2").
 
    - (* Case Storing (n + 1), invariant was in state F F1 1/3 *)
      ...
      iDestruct "Own" as "(OwnF & OwnF1 & Own1/3_I)".
      iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
      iMod ("Iclose" with "[Hl OwnF_1 OwnF1 Own1/3_I]") as "_".
      { iRight. iLeft. iFrame. }
      iApply ("Post" with "OwnF_2").
    - (* Case storing (n + 1), invariant was in state F F2 2/3 *)
    ...
    iDestruct "Own" as "(OwnF & OwnF2 & Own2/3_I)".
    iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
    iMod (own_update γ2 F2 F1 with "OwnF2") as "OwnF1". -- first (apply SFFFRA_update_4).
    iMod ("Iclose" with "[Hl OwnF_1 OwnF1 Own1/3]") as "_".
    { iRight. iLeft. iFrame. }
    iApply ("Post" with "OwnF_2").
    - (* Case storing (n + 2), invariant was in state S ST *)
    (* Impossible case *)
      iDestruct "Own" as "[OwnS OwnST]".
      iDestruct (own_SF_false with "[$OwnS $OwnF]") as "HFalse".
      done.
      - (* Case storing (n + 2), invariant was in state F F1 1/3 *)
      (* 3rd dijunct. We add our token to the invariant. *)
      iDestruct "Own" as "(OwnF_I & OwnF1_I & Own1/3_I)".
      iCombine "Own1/3 Own1/3_I" as "Own2/3".
      rewrite  div_2_3.
      iMod (own_update _ _ F2 with "OwnF1_I") as "OwnF2"; first (apply SFFFRA_update_2).
       iMod ("Iclose" with "[Hl OwnF_I OwnF2 Own2/3]") as "_".
      { iNext. iRight. iRight. iFrame. iLeft. iFrame. }
      iApply ("Post" with "OwnF"). 
   - (* Storing (n + 2), invariant was in state F F2 2/3 *)
      (* 3rd disjunct. Two threads have already contributed 1/3 tokens each. we do
         not contribute our token to the invariant. *)
         iDestruct "Own" as "(OwnF_I & OwnF2_I & Own2/3_I)".
         (* iMod (own_update _ _ F with "OwnS2_I") as "OwnF2"; first (apply SFRA_update). *)
         (* iDestruct (own_SFRA_FF with "OwnF2") as "[OwnF2_1 OwnF2_2]". *)
         iMod ("Iclose" with "[Hl OwnF_I OwnF2_I Own2/3_I]") as "_"; iFrame.
         { iNext. iRight. iRight. iLeft. iFrame. }
         iApply ("Post" with "OwnF").
   - (* Storing (n + 3), invariant was in state S ST*)
     (* Impossible case *)
     iDestruct "Own" as "[OwnS OwnST]".
     iDestruct (own_SF_false with "[$OwnS $OwnF]") as "HFalse".
     done.
   - (* Storing (n + 3), invariant was in state F F1 1/3 *)
     (* Impossible. *)
     iDestruct "Own" as "(OwnF_ & OwnF1_ & _)".
     iDestruct (own_SFFF_false_4 with "[$OwnF2 $OwnF1_]") as "HFalse".
     done.
   - (* Storing (n + 3), invariant was in state F F2 2/3 *)
     (* This case represents the situation where two threads have run
         ...
         { iRight. iRight. iNext. iRight. iFrame.  }
         iApply ("Post" with "OwnF").
...
Lemma div3_1_2 (γ: gname): (own γ (1)%Qp ⊣⊢ own γ (2/3)%Qp ∗ 
                                                         own γ (1/3)%Qp)%I.
Proof.
Admitted.

Lemma div2_1_1 (γ: gname): (own γ (2/3)%Qp ⊣⊢ own γ (1/3)%Qp ∗ 
                                                         own γ (1/3)%Qp)%I.
Proof.
Admitted.
...
Lemma parallel_incr_spec (l : loc) (n : Z):
  {{{ l ↦ #n }}} ((incr l) ||| (incr l)) ||| (incr l) ;; !#l {{{v, RET #v; 
                        ⌜(v = n+1)%Z⌝ ∨ ⌜(v = n+2)%Z⌝ ∨ ⌜(v = n+3)%Z⌝ }}}.
Proof using Type* N.
  iIntros (Φ) "Hl Post".
  (* We start by allocating the start token S and the fraction 1. *)
  iMod (own_alloc S) as (γ1) "OwnS"; first done.
  iMod (own_alloc ST) as (γ2) "OwnST"; first done.
  iMod (own_alloc 1%Qp) as (γ3) "Own1"; first done.
  (* We allocate the invariant in the first state, where we have to provide an S
  token. *)
  iMod (inv_alloc N _ (incr_inv l γ1 γ2 γ3 n) with "[Hl OwnS OwnST]") as "#Inv".
  { iLeft. iFrame. }
  ...
  - iDestruct "Own1" as "(Own2/3_1 & Own1/3_2)".
    ...
    wp_smart_apply (wp_par (λ _ , own γ1 F)%I (λ _ , own γ1 F)%I with "[Own2/3_1] [Own1/3_2]").
    (* We can now for each thread apply our incr_spec specification. *)
    +  rewrite div2_1_1.
       iDestruct "Own2/3_1" as "(Own1/3_1 & Own1/3_2)".
       wp_smart_apply (wp_par (λ _ , own γ1 F)%I (λ _ , own γ1 F)%I with "[Own1/3_1] [Own1/3_2]").
       ++ iApply (incr_spec l n γ1 γ2 γ3 with "[Inv Own1/3_1]"); first by iFrame.
       iNext. iIntros (_) "H". done.
       ++ iApply (incr_spec l n γ1 γ2 γ3 with "[Inv Own1/3_2]"); first by iFrame.
       iNext. iIntros (_) "H". done.
       ++ iIntros (v1 v2). iIntros "[OwnF _]". iNext. done.
    
    + iApply (incr_spec l n γ1 γ2 γ3 with "[Inv Own1/3_2]"); first by iFrame.
    iNext. iIntros (_) "H". done.
    + iIntros (v1 v2) "[OwnF _]".
      ...
      * (* Case 2: ℓ ↦ #(n + 1), F F1 1/3
        ...
        iMod ("Iclose" with "[I Own]") as "_".
        { iRight. iLeft. iFrame. }
        iModIntro. iApply "Post". iLeft. done.
      * (* Case 3: ℓ ↦ #(n + 2), F F2 2/3
           This state corresponds to the situation where both threads have
           incremented the value subsequently. *)
        wp_load.
        iMod ("Iclose" with "[I Own]") as "_".
        { iNext. iRight. iRight. iLeft. iFrame. }
        iModIntro. iApply "Post". iRight. iLeft. done.
     * (* Case 4: ℓ ↦ #(n + 3), F 1
        This state corresponds to the situation where both threads have
        incremented the value subsequently. *)
            wp_load.
            iMod ("Iclose" with "[I Own]") as "_".
            { iNext. iRight. iRight. iRight. iFrame. }
            iModIntro. iApply "Post". iRight. iRight. done.
Qed.
