...
Definition incr_inv (ℓ : loc) (γ1 γ2 γ3 : gname) (n : Z) : iProp :=
    ℓ ↦ #n ∗ own γ1 S  ∗ own γ2 S 
  ∨ ℓ ↦ #(n+1) ∗ own γ1 F ∗ own γ2 S ∗ own γ3 (1/3)%Qp
  ∨ ℓ ↦ #(n+2) ∗ own γ1 F ∗ own γ2 S ∗ own γ3 (2/3)%Qp
  ∨ ℓ ↦ #(n+3) ∗ own γ1 F ∗ own γ2 F ∗ own γ3 1%Qp.
...
Proof.
  ...
  iApply ((wp_wand _ _ _ (fun v => (⌜v = #n⌝ ∨ 
                                    ⌜v = #(n+1)⌝ ∗ own γ1 F ∨ 
                                    ⌜v = #(n+2)⌝ ∗ own γ1 F) ∗ own γ3 (1 / 3)%Qp)%I with "[Own1/3]")).
  { (* The first triple about the result of !ℓ *)
    ...
    - (* Case 1: l ↦ #n ∗ own γ1 S ∗  own γ2 S *)
      ...
      { iLeft. iFrame. }
      iFrame. iLeft. done.
    - (* Case 2: l ↦ #(n + 1) ∗ own γ1 F ∗ own γ2 S ∗ own γ3 (1 / 3)%Qp *)
      (* This is analogous to the first case. *)
      iDestruct "I" as "(Hl & OwnF & OwnS & Own1/3_I)".
      ...
      wp_load.
      iMod ("Iclose" with "[Hl OwnF_1 OwnS Own1/3_I]") as "_".
      { iRight. iLeft. iFrame. }
      iFrame. iRight. iLeft. iFrame. done.
      - (* Case 3: l ↦ #(n + 2) ∗ own γ1 F ∗ own γ2 S ∗ own γ2 (2 / 3)%Qp *)
      (* This is analogous to the first case. *)
      iDestruct "I" as "(Hl & OwnF1 & OwnS2 & Own2/3_I)".
      (* Similar to previous case, but there are two Fs to duplicate. *)
      iDestruct (own_SFRA_FF with "OwnF1") as "[OwnF1_1 OwnF1_2]".
      (* iDestruct (own_SFRA_FF with "OwnF2") as "[OwnF2_1 OwnF2_2]". *)
      wp_load.
      iMod ("Iclose" with "[Hl OwnF1_1 OwnS2 Own2/3_I]") as "_".
      { iNext. unfold incr_inv. iRight. iRight. iLeft. iFrame. } iModIntro.
      iFrame. iRight. iFrame. iRight.  done.
    - (* Case 4: l ↦ #(n + 3) ∗ own γ1 F ∗ ∗ own γ2 F ∗ own γ2 1%Qp
      ...
      iDestruct "I" as "(Hl & OwnF1 & OwnF2 & Own1)".
  ...
  { (* The second triple about storing the incremented v *)
    ...
    iDestruct "Hv" as "[-> | [[-> OwnF1] | (-> & OwnF1 )]]";
      ...
      iInv N as ">[I | [I | [I | I]]]" "Iclose";
      (* For case 4 of the invariant (ℓ ↦ #(n + 3) ∗ own γ1 F ∗ own γ1 F ∗ own γ2
      ...
      try (
          iDestruct "I" as "(Hl & OwnF1_I & OwnF2_I & Own1)";
          iCombine "Own1 Own1/3" as "Own4/3";
          iDestruct (own_valid with "Own4/3") as "%Hinvalid";
          contradiction
      );
    ...
    - (* Case Storing (n + 1), invariant was in state S S *)
      ...
      iDestruct "Own" as "[OwnS1 OwnS2]".
      iMod (own_update _ _ F with "OwnS1") as "OwnF"; first (apply SFRA_update).
      (* We then duplicate F, because we need to pass one F to the conclusion too. *)
      (* iMod (own_alloc S) as (γ2) "OwnS"; first done. *)
      iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
      iMod ("Iclose" with "[Hl OwnF_1 OwnS2 Own1/3]") as "_".
      
      { iRight. iLeft. iFrame. }
      iApply ("Post" with "OwnF_2").
 
    - (* Case Storing (n + 1), invariant was in state F S 1/3 *)
      ...
      iDestruct "Own" as "(OwnF & OwnS & Own1/3_I)".
      iDestruct (own_SFRA_FF with "OwnF") as "[OwnF_1 OwnF_2]".
      iMod ("Iclose" with "[Hl OwnF_1 OwnS Own1/3_I]") as "_".
      { iRight. iLeft. iFrame. }
      iApply ("Post" with "OwnF_2").
    - (* Case storing (n + 1), invariant was in state F S 2/3 *)
    ...
    iDestruct "Own" as "(OwnF1 & OwnS2 & Own2/3_I)".
    iDestruct (own_SFRA_FF with "OwnF1") as "[OwnF1_1 OwnF1_2]".
    iMod ("Iclose" with "[Hl OwnF1_1 OwnS2 Own1/3]") as "_".
    { iRight. iLeft. iFrame. }
    iApply ("Post" with "OwnF1_2").
    - (* Case storing (n + 2), invariant was in state S S *)
      (* Impossible case *)
      iDestruct "Own" as "[OwnS1 OwnS2]".
      iDestruct (own_SF_false with "[$OwnS1 $OwnF1]") as "HFalse".
      done.
      - (* Case storing (n + 2), invariant was in state F S 1/3 *)
      (* 3rd dijunct. We add our token to the invariant. *)
      iDestruct "Own" as "(OwnF1_I & OwnS2_I & Own1/3_I)".
      iCombine "Own1/3 Own1/3_I" as "Own2/3".
      rewrite  div_2_3.

       iMod ("Iclose" with "[Hl OwnF1_I OwnS2_I Own2/3]") as "_".
      { iNext. iRight. iRight. iFrame. iLeft. iFrame. }
      iApply ("Post" with "OwnF1"). 
   - (* Storing (n + 2), invariant was in state F S 2/3 *)
      (* 3rd disjunct. Two threads have already contributed 1/3 tokens each. we do
         not contribute our token to the invariant. *)
         iDestruct "Own" as "(OwnF1_I & OwnS2_I & Own2/3_I)".
         (* iMod (own_update _ _ F with "OwnS2_I") as "OwnF2"; first (apply SFRA_update). *)
         (* iDestruct (own_SFRA_FF with "OwnF2") as "[OwnF2_1 OwnF2_2]". *)
         iMod ("Iclose" with "[Hl OwnF1_I OwnS2_I Own2/3_I]") as "_"; iFrame.
   ...
   - (* Storing (n + 3), invariant was in state F S 1/3 *)
     (* Impossible. *)
     iDestruct "Own" as "[OwnS1 OwnS2]".
     iDestruct (own_SF_false with "[$OwnS1 $OwnF1]") as "HFalse".
